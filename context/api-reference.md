...
- ⚠️ **`ga` (y el resto de las estadísticas) está congelado al cierre de la fase de grupos, no es un acumulado de todo el torneo.** Verificado empíricamente: equipos que ya jugaron partidos de eliminatoria (octavos, cuartos, semifinal) siguen mostrando `mp: "3"` en `/get/groups` — solo cuenta los 3 partidos de fase de grupos, sin actualizarse con los goles recibidos después. Esto es el comportamiento esperado del endpoint (es la tabla de posiciones de grupo, no una estadística histórica completa), y es exactamente el dato que RF-EM-01 pide usar — no es un bug ni algo que "corregir" sumando datos de `/get/games`. Sí conviene dejarlo explícito en la UI (ej. etiqueta "GC en fase de grupos" en vez de solo "GC") para que no se confunda con el desempeño defensivo de todo el torneo en la defensa oral.
