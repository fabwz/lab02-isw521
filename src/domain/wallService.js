...
export const buildWallRanking = (groups, teams, games, { forcedFailureIndex = null } = {}) => {
  ...
  const ranking = top5.map((registro, indice) => {
    ...
    let opponentInfo;
    try {
      opponentInfo = resolveNextOpponent(registro.teamId, games, teamsById, indice === forcedFailureIndex);
    } catch (error) {
