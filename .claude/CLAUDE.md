...
└── src/
    ...
    │       - ordenar por local\_date
    │       - cruzar stadium\_id → datos de estadio
...
Este comportamiento implica que el render de tarjetas debe ser **incremental/parcial**: debe existir una función que actualice solo el campo de estadio de una tarjeta específica por su `stadium\_id`/`game\_id`, sin re-renderizar toda la lista.
...
2. Usuario elige equipo → filtrar `GET /get/games` donde `home\_team\_id === team.id OR away\_team\_id === team.id`.
3. Ordenar partidos filtrados por `local\_date` ascendente.
4. Por cada partido, cruzar `stadium\_id` contra `GET /get/stadiums` → obtener `city\_en`, país, `capacity`.
...
6. Calcular `count(distinct city\_en)` sobre los partidos encontrados y mostrarlo.
...
* \[ ] Filtro de partidos por equipo (home o away), ordenado por `local\_date`.
